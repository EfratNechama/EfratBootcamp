...
def save_image( data_arr, path_to_save):
    ...
    array = cv2.cvtColor(data_arr, cv2.COLOR_RGB2BGR)
    # save to PNG file
    return cv2.imwrite(path_to_save, array)
...
def load_cifar10():
    ...
    image_data = np.reshape(image_data, (len(image_data), 3, 32, 32))
    for i in image_data:
        save_image(i, df.iloc[i]["image_path"] + df.iloc[i]["image_name"])
...
def load_x_labels_from_cifar100(categorys_numbers: List[int]):
    ...
    return cifar100
def update_csv_after_split(x_train, x_validation, x_test):
    df=pd.read_csv(config.csv_path)
    print(x_train['index'])
    df.iloc[x_train['index']]['split']='train'
...
def split_train_test_validation():
    ...
    #update_csv_after_split(x_train, x_validation, x_test)
